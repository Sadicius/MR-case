...
$(document).on("keydown", function () {
    switch (event.keyCode) {
        case 27: // ESC
            if (escUsable) {
                $.post("https://MR-case/closeMenu", JSON.stringify());
                $(".fullBackGround").hide();
                $(".generalSection").hide();
                $(".caseOpeningSection").hide();
                $(".topBar").hide();
        ...
        if (firstOpen) {
            ...
            function processRequest(e) {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    ...
                    $(".profilePhotoMArwenn").attr("src", profilePhoto);
                }
            }
        } else {
            $(".profilePhotoMArwenn").attr("src", profilePhoto);
